...
// Category icon mapping by name keyword
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    // Trigger a reload every time this tab is shown
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _SectionHeader(
                        ...
                        fg: fg),
                    const SizedBox(height: 14),
                    _buildOffers(state, primary, accent, secondary, card, fg,
                        muted),
  ...
  Widget _buildCategories(HomeState state, Color primary, Color card,
    ...
    if (state is HomeLoaded && state.categories.isNotEmpty) {
      final cats = state.categories.take(4).toList();
      return Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Row(
  ...
  Widget _buildPharmacies(HomeState state, Color primary, Color accent,
    ...
    if (state is HomeLoaded && state.pharmacies.isNotEmpty) {
      return Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          children: state.pharmacies.take(3).map((p) {
            return Padding(
  ...
  Widget _buildOffers(HomeState state, Color primary, Color accent,
    ...
    if (state is HomeLoaded && state.offers.isNotEmpty) {
      return Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          children: state.offers.take(3).map((o) {
            return Padding(
              padding: const EdgeInsets.only(bottom: 10),
              child: OfferCard(
  ...
  Widget _buildPillReminderSummary(
      Color primary, Color accent, Color card, Color fg, Color muted) {
    final dark = Theme.of(context).brightness == Brightness.dark;
    return Container(
      margin: const EdgeInsets.fromLTRB(20, 0, 20, 12),
